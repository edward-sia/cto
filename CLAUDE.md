...
Live provider verification is separate from `npm test`. Use `npm run test:live-providers` with real keys to check normalized provider responses, CTO-style JSON parsing, and a real `TaskAnalyzer` call. Use `CTO_LIVE_PROVIDER_FILTER=openai,gemini`, `CTO_LIVE_<PROVIDER>_MODEL`, and `CTO_LIVE_PROVIDER_TIMEOUT_MS` to narrow or tune live runs without editing code. For EdenAI, use `EDENAI_API_KEY` and optionally `CTO_LIVE_EDENAI_MODEL`. Do not print API key values in logs.
